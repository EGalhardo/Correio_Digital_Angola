...
export default function App() {
  ...
  const [showExecutivePilot, setShowExecutivePilot] = useState(false);
      ...
      setTimeout(() => {
        ...
        if (canvas) {
          try {
            ...
            console.error('Failed to execute QRCode generation:', err);
          }
        }
      }, 150);
    }
  ...
  return (
    <main className={`min-h-screen bg-bg text-primary md:flex md:gap-5 md:p-5 font-sans selection:bg-primary selection:text-white transition-all ${emergencyMode && isGovMode ? 'pt-[32px] md:pt-[44px]' : ''}`}>
      ...
      <div className="flex-1 md:bg-white md:rounded-[24px] md:shadow-xl md:border border-line/60 md:overflow-hidden flex flex-col min-h-screen md:min-h-0 relative">
        <div className={emergencyMode && isGovMode ? 'md:mt-0' : ''}>
          <Header 
            ...
            onClickExecutivePilot={() => setShowExecutivePilot(true)}
            NotificationDropdown={() => (
              <NotificationDropdown 
                showNotifications={showNotifications} 
                setShowNotifications={setShowNotifications} 
      ...
      <AIChatAssistant 
        ...
        onStartExecutivePilot={() => setShowExecutivePilot(true)}
      />
      ...
      {/* --- TOUR EXECUTIVE AUTÓNOMO AI --- */}
      <ExecutivePilotMode
        isOpen={showExecutivePilot}
        onClose={() => setShowExecutivePilot(false)}
        onNavigateTab={(tabId) => {
          setTab(tabId);
          setSelectedDoc(null);
        }}
