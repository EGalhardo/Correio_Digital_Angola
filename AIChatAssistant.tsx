...
const WELCOME_MESSAGES = {
  pt: {
    welcome: "Seja muito bem vindo ao Correio Digital Angola. Transformamos o seu Bilhete de Identidade no seu endereço digital oficial, garantindo a receção de notificações chanceladas diretamente no telemóvel com validade jurídica, libertando-o de deslocações físicas. A nossa plataforma baseia-se em cinco pilares fundamentais: o Comando Principal para alertas críticos, a área de Correio de diálogo direto com as instituições, a Inteligência Artificial para simplificar termos jurídicos e atuar por voz, a Carteira Digital Segura e a Segurança Biométrica. Pretende que eu faça uma apresentação interactiva em todas as páginas da plataforma Correio Digital Angola?",
    admin: "Saudações executivas. Seja muito bem-vindo ao Centro de Comando do Correio Digital Angola. O nosso barramento unificado moderniza a comunicação administrativa, ligando 105 entidades de Soberania com força probatória e auditoria cibernética SOC permanente. Pretende que eu faça uma apresentação interactiva em todas as páginas da plataforma Correio Digital Angola?",
    inst: "Seja muito bem-vindo ao Balcão Institucional do Correio Digital Angola. A nossa solução moderniza o atendimento, capacitando a sua entidade com canais de trâmite cifrado, emissão de certidões autênticas e treino dedicado do vosso próprio assistente IA. Pretende que eu faça uma apresentação interactiva em todas as páginas da plataforma Correio Digital Angola?"
  },
  um: {
...
export function AIChatAssistant({ 
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    if (
      normalizedText === "sim" ||
      normalizedText.includes("sim, pretendo") ||
      normalizedText.includes("sim, quero") ||
      normalizedText.includes("com certeza") ||
      normalizedText.includes("claro") ||
      normalizedText.includes("pode fazer") ||
      normalizedText.includes("fazer apresentação") ||
      normalizedText.includes("fazer apresentacao") ||
      normalizedText.includes("apresentação") ||
      normalizedText.includes("apresentacao") ||
      normalizedText.includes("inicia") ||
      normalizedText.includes("como funciona") ||
      normalizedText.includes("explicar tudo") ||
      normalizedText.includes("explicar as páginas") ||
      normalizedText.includes("explicar as paginas") ||
      normalizedText.includes("como é que a minha área funciona") ||
      normalizedText.includes("como e que a minha area funciona") ||
      normalizedText.includes("piloto") ||
      normalizedText.includes("tour")
    ) {
      if (onStartExecutivePilot) {
        onStartExecutivePilot();
        onClose();
        const pilotMsg = "Com certeza! Vou iniciar a Apresentação Interactiva e Explicativa da plataforma Correio Digital de Angola. A visualização da página acompanhará em perfeita sintonia.";
        if (iaLiveActive) speak(pilotMsg);
        return;
      }
    }
