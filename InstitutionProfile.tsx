...
export const InstitutionProfile: React.FC<InstitutionProfileProps> = ({
  ...
  return (
    <section className="space-y-6 text-slate-950 animate-fade-in font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-5 mb-2 gap-4 font-sans min-w-0">
        <div className="flex items-center gap-3.5 text-left min-w-0">
          <button 
            onClick={() => setTab('home')}
            className="flex items-center justify-center w-10 h-10 bg-white border-2 border-[#d1dbe5] rounded-full text-[#384e6e] hover:bg-slate-50 transition-all shadow-md cursor-pointer hover:scale-105 active:scale-95 shrink-0 mt-1"
            aria-label="Voltar"
          ...
          <div className="min-w-0 truncate">
            <span className="text-xs uppercase font-bold tracking-widest text-slate-400 block truncate">Minha Conta</span>
            <h1 className="text-2xl md:text-3xl font-black text-slate-950 tracking-tight truncate m-0">Perfil do Utilizador</h1>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-0.5 truncate m-0">Gerencie suas informações pessoais e preferências de acesso</p>
          </div>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 bg-[#f0f4ff] border border-blue-150 rounded-full text-blue-700 font-extrabold text-[11px] uppercase tracking-wider shrink-0">
