...
import { resolveInstitutionCode } from '../../services/supabaseService';
...
export function VideoSessionPanel({ message, addAuditLog }: VideoSessionPanelProps) {
  const { user, appMode, activeProfile } = useSession();
      ...
      if (message) {
        ...
        const roleFiltered = allSessions.filter(s => {
          ...
          if (appMode === 'institution') {
            const instCode = resolveInstitutionCode(activeProfile.institutionName);
            return s.hostBi?.includes(instCode) || s.roomName?.includes(instCode.toLowerCase());
          }
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const instActiveCode = resolveInstitutionCode(activeProfile.institutionName);
      const newSession = await VideoSessionService.createSession({
        ...
        status: 'disponivel',
        hostBi: appMode === 'institution' ? 'INST-' + (message?.org || instActiveCode) : 'ADM-CDA',
        hostName: appMode === 'institution' ? (message?.org || activeProfile.institutionName) : 'Administrador do Portal',
        guestBi: user.bi,
        guestName: user.name,
        scheduledFor,
      });
  ...
  const handleJoinVideo = async () => {
    ...
    const userName = appMode === 'user' ? user.name : appMode === 'institution' ? activeProfile.institutionName : 'Administração Central CDA';
